...
class pytestARAP:
    ...
    def getVector_S(self):
        ...
        sx = np.zeros(shape=((self.M+self.N),1))
        lbx = sx
        f11 = np.matmul(sx.T,Q)
        f1 = np.matmul(f11,sx)
        f2 = np.matmul(sx.T,q1)
        fun = f1 + f2

        bounds = Bounds(lbx, h1.T)
        Gx = -1*G1
        linear_constr = LinearConstraint(G, h1.T, h1.T)
        x0 = np.zeros(shape=((self.M+self.N),1))

        res = minimize(fun, x0, method='trust-constr',jac=fun_der, constraints=[linear_constr], options={'verbose':1}, bounds = bounds)
        print(res.x)
        print("******************")
